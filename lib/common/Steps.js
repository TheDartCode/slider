...
var Steps = function Steps(_ref) {
  ...
  var range = max - min;
  var elements = calcPoints(vertical, marks, dots, step, min, max).map(function (point, index) {
    var _classNames;

    var offset = canonical ? Math.abs(point - min) / range * 100 + '%' : index / (Object.keys(marks).length - 1) * 100 + '%';
